...
export default function ProductItem({item}: { item: ProductDto }) {
    ...
    return (
        <div
            key={item.id}
            className={"flex text-sm md:text-base justify-center items-center flex-col gap-y-4 p-4 bg-white rounded-md"}>
            ...
            <div className={"flex flex-col gap-y-5  items-center"}>
                ...
                <div>
                    <Link className={` text-white py-2 px-8 rounded-md  cursor-pointer
                        bg-green-400 cursor-pointer"}`} to={{pathname: `productDetail/${item.id}`}}>به روز رسانی
                    </Link>
